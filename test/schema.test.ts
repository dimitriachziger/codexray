...
import { buildMultiSessionReport } from "../src/report.js";
...
test("generated single- and multi-session reports satisfy the JSON schema", async () => {
  ...
  for (const report of [simple, complex, multi]) {
    assert.equal(
      validate(report),
      true,
